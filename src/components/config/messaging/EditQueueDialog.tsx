...
interface EditQueueDialogProps {
  ...
  onSave: (queue: Queue, originalName: string) => void;
  queue: Queue | null;
...
export function EditQueueDialog({
  ...
  // Initialize form when dialog opens or queue changes
  useEffect(() => {
    if (open && initialQueue) {
      setQueue({ ...initialQueue });
      setOriginalName(initialQueue.name);
      setErrors({});
      setTouched({});
    }
  }, [open, initialQueue]);
  ...
  const validate = (): boolean => {
    ...
    if (!queue.name || queue.name.trim() === '') {
      ...
      } else if (queue.name !== originalName && existingQueueNames.includes(queue.name)) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Edit SQS Queue</DialogTitle>
          <DialogDescription>
            Update queue configuration and settings
          </DialogDescription>
        </DialogHeader>
